  ...
  try {
    ...
    if (employeesList) {
      employeesList.forEach((emp: any) => {
        if (emp.id) validEmployeeIds.add(emp.id.toString().trim().toLowerCase());
        if (emp.employee_id) validEmployeeIds.add(emp.employee_id.toString().trim().toLowerCase());
      });
    }
    ...
    console.log(`[sync-monthly-attendance] Querying logs from ${fromDate} to ${toDate} for serials: ${serials.join(", ")}`);
